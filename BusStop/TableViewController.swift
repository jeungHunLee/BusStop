...
class TableViewController: UITableViewController, XMLParserDelegate {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if busInfo[(indexPath as NSIndexPath).row].arrTime > 60 {
            ...
            //cell.textLabel.busNumber.text = busInfo[(indexPath as NSIndexPath).row].number
            cell.textLabel?.text = "\(busInfo[(indexPath as NSIndexPath).row].number)번| \(min)분 \(sec)초 후 도착할 예정"
        } else {
            cell.textLabel?.text = "\(busInfo[(indexPath as NSIndexPath).row].number)번| 잠시 후 도착할 예정"
        }
